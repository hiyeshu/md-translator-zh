...
# md-translator-zh - Markdown 英译中 VS Code 扩展
Node.js + VS Code Extension API + HTTPS

<directory>
out/ - 扩展核心逻辑 (9文件: extension, config, logger, translationViewer, translationManager, translationProviders, translationCache, markdownProcessor, statusBar)
assets/ - 静态资源 (icon.png)
dist/ - 打包产物 (.vsix)
.github/workflows/ - CI: tag 触发自动打包+发布 Open VSX+GitHub Release
</directory>

<config>
package.json - 扩展清单：命令、菜单、配置项、脚本、发布元数据
tsconfig.json - TypeScript 编译配置（当前无 .ts 源码，out/ 直接编辑）
DESIGN.md - Cursor 风格设计系统参考
</config>
